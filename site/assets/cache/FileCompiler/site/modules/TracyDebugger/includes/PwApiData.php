...
class TracyPwApiData extends \ProcessWire\WireData {
    ...
    public function getApiData($type) {
        ...
        if(!$apiData || \TracyDebugger::getDataValue('apiDataVersion') === null || $this->wire('config')->version != \TracyDebugger::getDataValue('apiDataVersion')) {
            ...
            elseif($type == 'hooks') {
                ...
                $apiData = array_merge(
                    $this->getApiHooks(\ProcessWire\wire('config')->paths->root.'wire/'),
                    $this->getApiHooks(\ProcessWire\wire('config')->paths->siteModules)
                );
            }
            ...
            if($cachedData && \TracyDebugger::getDataValue('apiDataVersion') !== null && $this->wire('config')->version != \TracyDebugger::getDataValue('apiDataVersion')) {
                ...
                $this->wire('cache')->save('TracyApiChanges', '~'.json_encode(\TracyDebugger::$apiChanges), \ProcessWire\WireCache::expireNever);
            ...
            $this->wire('cache')->save($cacheName, $apiData, \ProcessWire\WireCache::expireNever);
    ...
    private function getPHPFilenames($root, $excludeFilenames = array()) {
        ...
        foreach($iter as $path => $dir) {
            ...
            if(!$dir->isDir() && strpos(str_replace(\ProcessWire\wire('config')->paths->root, '', $path), DIRECTORY_SEPARATOR.'.') === false && preg_match($fileNamePattern, $path) && !in_array(basename($path), $excludeFilenames) ) {
                $paths[] = $path;
            }
        }
